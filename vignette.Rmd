...
## Envelope
- Bandpass filter using linear envelope
...
## Down sampling
```{r,echo=T,warning=F,message=F}
obj = emgR::downSample(obj,freq = 100)
obj
```

## Down sampled envelope
```{r,echo=T,warning=F,message=F}
matplot(obj$filter$emg,type="l")
```
...
- This part is build on `NMF` package
- `muscleSynergy2D.rankEst` explore best ranks
```{r,warning=F,message=F}
...
## Muscle synergy (2D-array)
- Perform NMF based on best fit rank
- Show synergy basis
```{r,warning=F,message=F}
...
<!-- ```{r,echo=T,warning=TRUE,message=FALSE} -->
<!-- obj = emgR::bandpass(obj,order = 4,filtFreq = c(5,100)) -->
<!-- obj -->
<!-- ``` -->
...
<!-- file_path = paste0(tempfile(tmpdir = "inst"),".pdf") -->
<!-- plot.emgR(obj,type="coh",files = file_path,cohRange=c(0,100)) -->
<!-- ``` -->

<!-- ## Visualize coherence functions -->
<!-- ```{r,include=T,echo=F,warning=F,message=F,out.width="70%",fig.align="center"} -->
...
<!-- ```{r,warning=F,message=F} -->
<!-- obj = emgR::cohNet(obj,mode = "bin",thres = 0.3,plot.it = FALSE) -->
<!-- ``` -->


<!-- ## Coherent muscle network construction -->
<!-- ```{r,warning=F,message=F,out.width="90%",fig.align="center"} -->
<!-- obj = emgR::cohNet(obj,mode = "bin",thres = 0.4,plot.it = TRUE) -->
<!-- ``` -->
